...
public class Interface extends Application {
    ...
    public void start(Stage stage){
        ...
        primeTF.setPrefSize(400,20);
        textFieldGroup.getChildren().add(primeTF);

        numberTF.setLayoutX(580);
        numberTF.setLayoutY(10);
        numberTF.setPromptText("Squared...");
        numberTF.setPrefSize(100,20);
        ...
        startButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent actionEvent) {
                try {
                    ...
                    logger.writeLog("Squared number: "+inputNumber,true);
                    logger.writeLog("x^2 = "+inputNumber+" (mod "+inputPrime+")",true);

                    Backend b = new Backend(inputPrime,inputNumber);
                    int result = b.run();
                    if(result==-1){
                        resultTextArea.setText(resultTextArea.getText()+"\nNo result for "
                                +"x^2 = "+inputNumber+" (mod "+inputPrime+")");
                    }else {
                        resultTextArea.setText(resultTextArea.getText() + ("\nRESULT: " + "X={" + result + ", "
                                + ((inputPrime - result) % inputPrime + inputPrime) % inputPrime + "}; "
                                + result + "^2 = " + inputNumber + " (mod " + inputPrime + ")"));
                    }
